...
import { roomQueryOptions, roomsQueryOptions, statusLabel, statusTone } from "@/lib/site-data";
import { ASSETS } from "@/lib/assets";
import { getRealRoomMedia } from "@/lib/room-media";
...
export const Route = createFileRoute("/heritage/rooms/$slug")({
  ...
  notFoundComponent: () => (
    <div className="max-w-3xl mx-auto px-6 py-24">
      ...
      <Link to="/heritage/rooms" className="underline mt-4 inline-block">
        Back to bedrooms
      </Link>
    </div>
  ),
  errorComponent: () => (
    <div className="max-w-3xl mx-auto px-6 py-24">Something went wrong loading this bedroom.</div>
  ),
  component: Page,
});
...
function Page() {
  ...
  const media = getRealRoomMedia(slug);
  ...
  return (
    <div className="site-container section-y">
      <Link to="/heritage/rooms" className="text-sm text-stone-600 underline">
        ← All bedrooms
      </Link>
      ...
      <div className="mt-16 grid lg:grid-cols-[1fr_1fr] gap-12">
        <div>
          ...
          <p className="mt-2 text-stone-600">
            Tell us a little about your loved one and we'll respond within one business day.
          </p>
        </div>
        <div className="p-6 rounded-2xl bg-white" style={{ border: "1px solid var(--h-border)" }}>
          <InquiryForm
            kind="room"
            design="heritage"
            sourcePath={`/heritage/rooms/${slug}`}
            roomSlug={slug}
            variant="heritage"
            submitLabel={`Inquire about ${room.name}`}
          />
        </div>
      </div>
      ...
      {others.length > 0 && (
        <div className="mt-20">
          ...
          <div className="grid sm:grid-cols-3 gap-6">
            {others.map((r) => {
              const cover = getRealRoomMedia(r.slug)[0];
              return (
                <Link
                  key={r.id}
                  to="/heritage/rooms/$slug"
                  params={{ slug: r.slug }}
                  className="rounded-xl overflow-hidden bg-white block"
                  style={{ border: "1px solid var(--h-border)" }}
                >
                  <img
                    src={cover?.url ?? ASSETS.room}
                    alt={`Heritage adult family home ${r.name}`}
                    className="w-full aspect-[4/3] object-cover"
                    loading="lazy"
                  />
                  <div className="p-4">
                    <h3 className="font-display">{r.name}</h3>
                  </div>
                </Link>
              );
            })}
